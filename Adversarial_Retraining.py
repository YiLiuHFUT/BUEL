import argparse
from sklearn.metrics import precision_recall_fscore_support
import numpy as np
import pandas as pd
from Standard_Training import test
import torch

# 设置随机种子
seed = 42
torch.manual_seed(seed)
torch.cuda.manual_seed(seed)
np.random.seed(seed)


# 模型训练
def train(model, train_data, val_data, lr, epoch, weight=1, path=None):
    best_loss, best_acc, best_F = 10, 0.5, 0.5
    print('train model')
    model = model.cuda()
    criterion = torch.nn.CrossEntropyLoss(weight=torch.Tensor([1, weight]), reduction='none').cuda()
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr)

    for epoch in range(epoch):
        train_loss = 0.0

        model.train()
        predictions = np.empty((0, 2))
        label_all = []
        for idx, (text, label, sample_type) in enumerate(train_data):
            # 设置benign和adversarial之间的trade-off
            sample_weight = []
            ben = 1.0
            adv = args.lamda
            pos = args.gamma
            neg = 1.0
            for i in range(len(sample_type)):
                if sample_type[i] == 'ben' and label[i].item() == 0:
                    sample_weight.append(ben * neg)
                elif sample_type[i] == 'ben' and label[i].item() == 1:
                    sample_weight.append(ben * pos)
                elif sample_type[i] == 'adv' and label[i].item() == 0:
                    sample_weight.append(adv * neg)
                elif sample_type[i] == 'adv' and label[i].item() == 1:
                    sample_weight.append(adv * pos)
            sample_weight = torch.tensor(sample_weight, requires_grad=True).cuda()

            train_x = torch.tensor(np.array([item.cpu().detach().numpy() for item in text])).cuda().T
            train_y = label.cuda()
            train_y = train_y.to(torch.int64)
            optimizer.zero_grad()
            pred = model(train_x)
            loss = criterion(pred, train_y)
            loss_mean = torch.mul(loss, sample_weight).mean()
            train_loss += loss_mean.item()
            loss_mean.backward()
            optimizer.step()
            # print([x.grad for x in optimizer.param_groups[0]['params']])  # 打印梯度
            predictions = np.vstack((predictions, pred.cpu().detach().numpy()))
            label_all += train_y.tolist()
        label_all = np.array(label_all)
        train_loss /= len(train_data)

        accuracy = np.sum(np.argmax(predictions, axis=1) == label_all) / len(label_all)
        _, _, f2, _ = precision_recall_fscore_support(label_all, np.argmax(predictions, axis=1), average='macro')
        print(
            f'epoch {epoch} train done! average train loss is {train_loss:.5f}, average train accuracy is {accuracy:.5f}, average train F-measure is {f2:.5f}')

        val_loss, val_acc, val_F = test(model, val_data)
        if best_loss > val_loss or (best_loss == val_loss and best_acc < val_acc):
            # 保存训练完成后的模型参数
            best_loss = val_loss
            print(
                f'Save model! Best validation accuracy is {val_acc:.5f}, Best validation F-measure is {val_F:.5f}')
            model.save_pretrained(path)
